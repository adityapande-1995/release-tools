...
ignition_collections.each { ign_collection ->
  ...
  OSRFGitHub.create(ignition_brew_ci_job,
                    "gazebosim/ign-${ign_collection_name}",
                    "main",
  ...
  steps {
    shell("""\
          ...
          for n in \${NIGHTLY_PACKAGES}; do
              ...
              python3 ./scripts/release.py \${dry_run_str} "\${n}" nightly "\${PASS}" --release-repo-branch main --nightly-src-branch \${src_branch} --upload-to-repo nightly > log-"\${n}" || echo "MARK_AS_UNSTABLE"
              echo " - done"
          done
